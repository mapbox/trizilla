...
tape('should force decompress to stored triangle size', function(t) {
    var compressed = JSON.parse(fs.readFileSync('./test/fixtures/compressed-features'));
...
tape('decompress correctly', function(t) {
    var compressed = JSON.parse(fs.readFileSync('./test/fixtures/compressed-features'));
    var DC = new trizilla_decompressor.Decompressor();
    t.ok(DC, 'decompressing')
    DC.decompress(compressed, function(err, data) {
        t.equals(err, null, 'should not error');
